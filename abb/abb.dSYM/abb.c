...
nodo_abb_t *abb_buscar_borrado(nodo_abb_t *raiz,nodo_abb_t **padre,const char *clave,abb_comparar_clave_t cmp){
	if(!raiz)
		return NULL;

	if(raiz->izq && !cmp(raiz->izq->clave,clave)){ 
		*padre = raiz;
		return raiz->izq;
	}
	else if (raiz->der && !cmp(raiz->der->clave,clave)){
		*padre = raiz;
		return raiz->der;
	}

	else if(cmp(clave,raiz->clave) < 0 && raiz->izq)
		return abb_buscar_borrado(raiz->izq,padre,clave,cmp);
	
	else if(raiz->der)
		return abb_buscar_borrado(raiz->der,padre,clave,cmp);		
...
void *abb_borrar(abb_t *arbol, const char *clave){
	...
	nodo_abb_t *borrado = abb_nodo_buscar(arbol->raiz,&padre,clave,arbol->comparar);
	// if(!borrado)
